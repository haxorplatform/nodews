...
var IMap = function() { };
IMap.__name__ = ["IMap"];
var ServiceDaemon = function() { };
ServiceDaemon.__name__ = ["ServiceDaemon"];
ServiceDaemon.main = function() {
	console.log("Process> Starting WebServiceDaemon");
	ServiceDaemon.http = nws.net.HTTPServiceManager.Create(8000);
	ServiceDaemon.http.Add("/contact/",example.service.Contact);
	ServiceDaemon.http.verbose = 0;
	var _g = 0;
	var _g1 = nodejs.NodeJS.get_process().argv;
	while(_g < _g1.length) {
		var a = _g1[_g];
		++_g;
		if(a.indexOf("-v") >= 0) ServiceDaemon.http.verbose = a.length - 1;
	}
	console.log("Process> Verbose Level [" + ServiceDaemon.http.verbose + "]");
	nodejs.NodeJS.get_process().on(nodejs.ProcessEventType.Exception,ServiceDaemon.OnError);
};
ServiceDaemon.OnError = function(p_error) {
...
var example = {};
example.service = {};
example.service.ContactStatus = function() { };
example.service.ContactStatus.__name__ = ["example","service","ContactStatus"];
var nws = {};
nws.service = {};
...
example.service.Contact = function(p_server) {
	nws.service.BaseService.call(this,p_server);
};
example.service.Contact.__name__ = ["example","service","Contact"];
example.service.Contact.__super__ = nws.service.BaseService;
example.service.Contact.prototype = $extend(nws.service.BaseService.prototype,{
	OnInitialize: function() {
		this.content = "application/json";
	}
	,OnExecute: function() {
		var resp = { };
		var status = example.service.ContactStatus.Error;
		this.manager.response.setHeader("Access-Control-Allow-Origin","*");
		if(this.manager.data == null) {
			...
			var mode;
			if(this.manager.data.mode == null) mode = "contact"; else mode = this.manager.data.mode;
			this.manager.Log("Contact> mode[" + mode + "]",2);
			switch(mode) {
			case "contact":
				status = this.SendContact(this.manager.data);
				break;
			}
			resp.success = false;
			resp.error = 1;
			switch(status) {
			case example.service.ContactStatus.Success:
				resp.success = true;
				resp.error = 0;
				resp.message = "Contact Sent!";
				break;
			case example.service.ContactStatus.Error:
				resp.message = "Unknown Error.";
				break;
			case example.service.ContactStatus.InvalidName:
				resp.message = "Invalid Name.";
				break;
			case example.service.ContactStatus.InvalidEmail:
				resp.message = "Invalid Email.";
				break;
			case example.service.ContactStatus.InvalidContent:
				resp.message = "Invalid Content.";
				break;
			}
		}
		var json_resp = JSON.stringify(resp);
		this.manager.response.write(json_resp);
	}
	,SendContact: function(p_data) {
		var _g = this;
		if(p_data == null) return example.service.ContactStatus.Error;
		var c_name;
		if(p_data.name == null) c_name = ""; else c_name = p_data.name;
		var c_email;
		if(p_data.email == null) c_email = ""; else c_email = p_data.email;
		var c_subject;
		if(p_data.subject == null) c_subject = ""; else c_subject = p_data.subject;
		var c_message;
		if(p_data.message == null) c_message = ""; else c_message = p_data.message;
		if(c_name == "") return example.service.ContactStatus.InvalidName;
		var email_regexp = new EReg("[A-Z0-9._%-]+@[A-Z0-9.-]+\\.[A-Z][A-Z][A-Z]?","i");
		if(!email_regexp.match(c_email)) return example.service.ContactStatus.InvalidEmail;
		if(c_message == "") return example.service.ContactStatus.InvalidContent;
		if(c_subject == "") c_subject = "Contact " + (function($this) {
			var $r;
			var _this = new Date();
			$r = HxOverrides.dateStr(_this);
			return $r;
		}(this));
		this.manager.Log("Contact> Sent name[" + c_name + "] mail[" + c_email + "] subject[" + c_subject + "] content[" + HxOverrides.substr(c_message,0,10) + "...]");
		var esopt = { };
		esopt.ssl = true;
		...
		require('emailjs').server.connect(esopt).send(msg,function(p_error,p_message) {
			if(p_error != null) _g.manager.Log("Contact> Mail Error [" + Std.string(p_error) + "]"); else _g.manager.Log("Contact> Mail from[" + msg.from + "] Sent!");
		});
		return example.service.ContactStatus.Success;
	}
	,__class__: example.service.Contact
});
var haxe = {};
haxe.ds = {};
...
var Enum = { };
example.service.ContactStatus.Success = "success";
example.service.ContactStatus.Error = "error";
example.service.ContactStatus.InvalidEmail = "invalid_email";
example.service.ContactStatus.InvalidName = "invalid_name";
example.service.ContactStatus.InvalidContent = "invalid_content";
